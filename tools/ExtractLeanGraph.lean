...
run_cmd do
  ...
  for (name, ci) in env.constants.toList do
    ...
    -- extract from the whole OpenGALib library (every concept gets a node)
    if !m.startsWith "OpenGALib" then continue
    ...
    let deps := allConsts.filter
        (fun d => d != name && ((constModule env d).map (·.startsWith "OpenGALib") |>.getD false))
      |>.map (·.toString)
