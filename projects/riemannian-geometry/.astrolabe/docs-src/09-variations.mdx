...
cross-references ("by Lemma 2.3", "Theor. 3.7, Chap. 3") so the dependency graph
...
### 2.2 Proposition (existence of a variation with given variational field)
...
### 2.3 Lemma (minimizing geodesics minimize energy)
...
minimizing geodesic (see Corollary 3.9 of Chapter 3). The converse is obvious.
$\square$

### 2.4 Proposition (formula for the first variation of the energy of a curve)
...
### 2.5 Proposition (geodesics as critical points of energy)
...
### 2.6 Remark
Proposition 2.5 furnishes a characterization of geodesics as critical points of
...
### 2.8 Proposition (formula for the second variation)
...
### 2.9 Remark
If the variation is not proper, the first term at the beginning of Proposition 2.8
...
### 2.12 Remark
The analogy mentioned in Remark 2.7 can be carried further, considering
...
As a result there exists an index $j$ such that $E_j''(0)<0$, which, by Lemma 2.3,
...
### 3.2 Corollary (finiteness of the fundamental group)
...
### 3.3 Corollary (positive sectional curvature bound)
...
### 3.6 Remark
The estimate for the diameter given by Theorem 3.1 cannot be improved. Indeed, the
...
### 3.8 Lemma (invariant vector of an orthogonal map)
...
*Proof of Theorem 3.7.* Suppose, to the contrary, that $f(q)\ne q$ for all $q\in M$.
...
Remark 2.9) and the fact that $\frac{\partial h}{\partial s}(0,t)=e_1(t)$,
...
$\tilde M$ is complete, by Corollary 3.3, $\tilde M$ is compact. Let
...
the orientation of $\overline M$. Since $n$ is odd, we can apply Theorem 3.7 which
