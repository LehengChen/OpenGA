...
cross-references ("by Proposition 2.2", "from Rauch's Theorem", "Chap. 11") so
...
### 2.2 Proposition (characterization of the cut point)
...
(Corollary 2.9 to the Index Theorem, Chap. 11), the cut point occurs at
...
### 2.4 Example (real projective space)
...
### 2.5 Example (flat torus, cylinder)
...
### 2.6 Remark
In Example 2.3 the situations (a) and (b) of Proposition 2.2 occur simultaneously;
in Example 2.4 situation (b) occurs before (a); in Example 2.5 only situation (b)
...
### 2.7 Corollary (symmetry of cut points)
...
*Proof.* By Proposition 2.2, either $q$ is conjugate to $p$, or there is a geodesic
...
### 2.8 Corollary (injectivity radius)
...
### 2.9 Proposition (continuity of the cut-distance)
...
is conjugate to $\gamma(0)$, so $\bar t\geq t_o$. Otherwise, by Proposition 2.2,
...
Proposition 2.2 gives $t_o\leq\bar t$; if $\sigma=\gamma$, a similar argument shows
$\gamma(\bar t)$ conjugate to $\gamma(0)$, so $\bar t\geq t_o$. $\square$

### 2.10 Corollary (the cut locus is closed)
...
function of Proposition 2.9. If $q$ is an accumulation point of $C_m(p)$, take
...
### 2.11 Corollary (cut point everywhere implies compact)
...
### 2.12 Proposition (geometry of a nearest cut point)
...
*Proof.* Let $\gamma$ be minimizing from $p$ to $q$. By Proposition 2.2 either $q$
...
$\ell(\sigma_s)<\ell(\sigma)$. If $\ell(\gamma_s)=\ell(\sigma_s)$, by Proposition 2.2
...
### 2.13 Proposition (injectivity radius vs. closed geodesics)
...
closed geodesic $\gamma$ of length $\ell=\ell(\gamma)<2\pi$. By Corollary 2.10 of
...
Consider $\Omega_{p,q}^c$ (Remark 2.12 of Chap. 11) and its finite-dimensional
...
Lemma 3.2 with $\bar M$ a sphere of curvature $\bar K=\inf_M K>1/4$, getting
...
### 3.4 Proposition (even-dimensional estimate)
...
Proposition 2.4 of Chap. 10, a contradiction. So $p$ is not conjugate to $q$; by
...
$q$; since $p$ is not conjugate to $q$, this contradicts Proposition 2.12. $\square$

### 3.5 Remark
By Synge's theorem (Chap. 9, Cor. 3.10) the manifolds satisfying the hypotheses of
Proposition 3.4 are simply connected. Conversely a simply connected manifold is
...
Proposition 3.4 is equivalent to assuming $M$ simply connected.

## §4 The Sphere Theorem

In what follows, whether $\dim M$ is even or odd plays no role.

### 4.1 Lemma (Berger [Br 1])
...
### 4.2 Lemma (covering by two balls; Tsukamoto [Ts])
Let $M^n$ be a compact, simply connected Riemannian manifold whose sectional
...
Under the conditions of Lemma 4.2, on each geodesic of length $\rho$ starting from
...
Lemma 4.2 gives $d(q,\gamma(s_o))<\rho$, so $f(s_o)<0$. Hence $f(s_1)=0$ for some
...
*Proof of Theorem 1.1.* Let $p,q\in M$ with $\operatorname{diam}M=d(p,q)$. Let $D_1$,
$D_2$ be the subsets of $M$ formed by all geodesic segments $\overline{pm}$ and
$\overline{qn}$ respectively, with $m,n$ from Lemma 4.3. By continuity of $m$ (Rem.
4.4), $D_1,D_2$ are closed; we claim $D_1\cup D_2=M$ and
$\partial D_1=\partial D_2=D_1\cap D_2$. Indeed, for $r\in M$, by Lemma 4.2 either
...
$\gamma(\pi/2)=e$; let $m$ be the point of Lemma 4.3 on the geodesic of $M$ through
...
$\partial D_1=\partial D_2=D_1\cap D_2$. By uniqueness of $m,n$ (Lemma 4.3),
