...
cross-references ("by Cartan's Theorem", "by Lemma 3.1 (ii)", "Chapter 7") so the
...
### 2.1 Definition (free homotopy class)
...
### 2.5 Definition (translation)
An isometry $f:\tilde{M}\to\tilde{M}$ without fixed points is said to be a
*translation* of $\tilde{M}$ if it leaves invariant some geodesic $\tilde{\gamma}$
...
### 3.1 Lemma (comparison for geodesic triangles, $K\leq 0$)
...
Since $K\leq 0$ and $T_c(\tilde{M})$ has zero curvature, we can apply Proposition
...
### 3.3 Lemma (uniqueness of the invariant geodesic)
...
triangles has its sum of interior angles $\geq\pi$, which contradicts Lemma 3.1
(ii). $\blacksquare$

### 3.4 Lemma (commuting isometry is a translation along the same geodesic)
...
### 3.5 Lemma (subgroup leaving a geodesic invariant is infinite cyclic)
...
subgroup with $H\neq\{e\}$. By Proposition 2.6 and by Lemmas 3.3 and 3.4, there
...
### 3.6 Example (surface of genus two times a circle)
...
### 3.9 Lemma (invariant geodesic forces non-compactness)
...
$\tilde{\gamma}$. Since $K\leq 0$, by Lemma 3.1 (i),
...
Lemma 3.5, $H_{k-2}$ is infinite cyclic.
