...
- `web/` — Astrolabe knowledge-network visualizer (Next.js + React). Also read `web/AGENTS.md` when working here.
...
6. When working in `web/`, also read and follow `web/AGENTS.md`.
...
- "review app", "review task", "atom edit", "pilot app" → invoke `openga-review-app`
- "math review", "review atom", "check OCR" → invoke `openga-math-review`
- "Lean", "formalize", "sorry" → invoke `openga-lean-formalize` (create if missing)
- "next task", "what should I do" → invoke `openga-task-router` (create if missing)
