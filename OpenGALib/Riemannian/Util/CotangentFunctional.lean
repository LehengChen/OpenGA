...
`koszulCotangentFunctional g v Y y : E →L[ℝ] ℝ` — a cotangent vector at
...
noncomputable def koszulCotangentScalar
  ...
  (1/2 : ℝ) * koszulFunctional g (fun _ : M => v) Y.toFun (fun _ : M => w) y
...
noncomputable def koszulCotangentFunctional
  ...
  LinearMap.toContinuousLinearMap
    { toFun := fun w => koszulCotangentScalar g v Y w y
      map_add' := by
        intro w₁ w₂
        ...
        have h_YZ₁ : MDifferentiableAt I 𝓘(ℝ, ℝ)
            (fun y' : M => g.metricInner y' (Y.toFun y') ((fun _ : M => w₁) y')) y :=
          g.metricInner_mdifferentiableAt hY_y h_const_w₁
        have h_YZ₂ : MDifferentiableAt I 𝓘(ℝ, ℝ)
            (fun y' : M => g.metricInner y' (Y.toFun y') ((fun _ : M => w₂) y')) y :=
          g.metricInner_mdifferentiableAt hY_y h_const_w₂
        have h_Z₁X : MDifferentiableAt I 𝓘(ℝ, ℝ)
            (fun y' : M => g.metricInner y' ((fun _ : M => w₁) y') ((fun _ : M => v) y')) y :=
          g.metricInner_mdifferentiableAt h_const_w₁
        ...
        have h_Z₂X : MDifferentiableAt I 𝓘(ℝ, ℝ)
          ...
          g.metricInner_mdifferentiableAt h_const_w₂
            ((SmoothVectorField.const (I := I) (M := M) v).smoothAt y)
        have h_add_factored :
            koszulFunctional g (fun _ : M => v) Y.toFun (fun _ : M => w₁ + w₂) y
              = koszulFunctional g (fun _ : M => v) Y.toFun (fun _ : M => w₁) y
                + koszulFunctional g (fun _ : M => v) Y.toFun (fun _ : M => w₂) y := by
          have h_sum_eq : ((fun _ : M => w₁ + w₂) : ∀ z : M, TangentSpace I z)
              = (fun _ : M => w₁) + (fun _ : M => w₂) := by
            funext z; rfl
          rw [h_sum_eq]
          exact koszul_add_right g (fun _ => v) Y.toFun (fun _ => w₁) (fun _ => w₂)
            y h_YZ₁ h_YZ₂ h_Z₁X h_Z₂X h_const_w₁ h_const_w₂
        show (1/2 : ℝ) * koszulFunctional g (fun _ : M => v) Y.toFun
              (fun _ : M => w₁ + w₂) y
            = (1/2 : ℝ) * koszulFunctional g (fun _ : M => v) Y.toFun (fun _ : M => w₁) y
              + (1/2 : ℝ) * koszulFunctional g (fun _ : M => v) Y.toFun (fun _ : M => w₂) y
        rw [h_add_factored]
      ...
      map_smul' := by
        ...
        have h_ZX : MDifferentiableAt I 𝓘(ℝ, ℝ)
            (fun y' : M => g.metricInner y' ((fun _ : M => w) y') ((fun _ : M => v) y')) y :=
          g.metricInner_mdifferentiableAt h_const_w h_const_v
        have h_smul_factored :
            koszulFunctional g (fun _ : M => v) Y.toFun (fun _ : M => c • w) y
              = c * koszulFunctional g (fun _ : M => v) Y.toFun (fun _ : M => w) y := by
          have h_eq : (fun _ : M => c • w : ∀ z : M, TangentSpace I z)
              = fun y' : M => (fun _ : M => c) y' • (fun _ : M => w) y' := by
            funext z; rfl
          rw [h_eq]
          exact koszul_smul_right g (fun _ => v) Y.toFun (fun _ => w)
...
@[simp]
lemma koszulCotangentFunctional_apply
    (g : RiemannianMetric I M)
    (v : E) (Y : SmoothVectorField I M) (y : M) (w : E) :
...
private theorem koszulCotangentScalar_mdifferentiableAt
    ...
    MDifferentiableAt I 𝓘(ℝ, ℝ) (fun y : M => koszulCotangentScalar g v Y w y) x := by
  classical
  ...
  -- Bundle-section smoothness witnesses, used by `g.metricInner_contMDiff`.
  ...
  -- Scalar functions for terms 1, 2, 3 via g.metricInner_contMDiff.
  have h_f_YW : ContMDiff I 𝓘(ℝ, ℝ) ∞
      (fun y' : M => g.metricInner y' (Y.toFun y') w) :=
    g.metricInner_contMDiff hY_bundle h_const_w_bundle
  have h_f_WV : ContMDiff I 𝓘(ℝ, ℝ) ∞
      (fun y' : M => g.metricInner y' w v) :=
    g.metricInner_contMDiff h_const_w_bundle h_const_v_bundle
  have h_f_VY : ContMDiff I 𝓘(ℝ, ℝ) ∞
      (fun y' : M => g.metricInner y' v (Y.toFun y')) :=
    g.metricInner_contMDiff h_const_v_bundle hY_bundle
  ...
  have hT1 : MDifferentiableAt I 𝓘(ℝ, ℝ)
      (fun y : M => mfderiv I 𝓘(ℝ, ℝ)
        (fun y' => g.metricInner y' (Y.toFun y') w) y v) x :=
    mfderiv_const_dir_smoothAt h_f_YW x v
  have hT2 : MDifferentiableAt I 𝓘(ℝ, ℝ)
      (fun y : M => mfderiv I 𝓘(ℝ, ℝ)
        (fun y' => g.metricInner y' w v) y (Y.toFun y)) x :=
    mfderiv_smoothDir_smoothAt h_f_WV (hY_E.contMDiffAt)
  have hT3 : MDifferentiableAt I 𝓘(ℝ, ℝ)
      (fun y : M => mfderiv I 𝓘(ℝ, ℝ)
        (fun y' => g.metricInner y' v (Y.toFun y')) y w) x :=
    mfderiv_const_dir_smoothAt h_f_VY x w
  have hT4 : MDifferentiableAt I 𝓘(ℝ, ℝ)
      (fun y : M => g.metricInner y (mlieBracket I (fun _ : M => v) Y.toFun y) w) x :=
    g.metricInner_mdifferentiableAt h_mlb_vY h_const_w_y
  have hT5 : MDifferentiableAt I 𝓘(ℝ, ℝ)
      (fun y : M => g.metricInner y (mlieBracket I Y.toFun (fun _ : M => w) y) v) x :=
    g.metricInner_mdifferentiableAt h_mlb_Yw h_const_v_y
  have hT6 : MDifferentiableAt I 𝓘(ℝ, ℝ)
      (fun y : M => g.metricInner y
        (mlieBracket I (fun _ : M => v) (fun _ : M => w) y) (Y.toFun y)) x :=
    g.metricInner_mdifferentiableAt h_mlb_vw hY_y
...
theorem koszulCotangentFunctional_smoothAt
    ...
    MDifferentiableAt I 𝓘(ℝ, E →L[ℝ] ℝ)
      (fun y : M => koszulCotangentFunctional g v Y y) x := by
  -- Componentwise lift: for each basis element b_i, the scalar
  -- (fun y => koszulCotangentFunctional g v Y y (b_i)) = (fun y => koszulCotangentScalar g v Y b_i y)
  ...
  intro i
  show MDifferentiableAt I 𝓘(ℝ, ℝ) (fun y : M => koszulCotangentFunctional g v Y y (basis i)) x
  have h_eq : (fun y : M => koszulCotangentFunctional g v Y y (basis i))
      = (fun y : M => koszulCotangentScalar g v Y (basis i) y) := by
    funext y
    exact koszulCotangentFunctional_apply g v Y y (basis i)
  rw [h_eq]
  exact koszulCotangentScalar_mdifferentiableAt g v Y (basis i) x
