import OpenGALib.Riemannian.Exponential.Smoothness.MfderivZero
...
/-- **Math.** The chart-pushed exponential map at `p`. -/
private def chartedExpAt (g : RiemannianMetric I M) (p : M) : E → E :=
  fun v => (extChartAt I p) (expMap (I := I) g p (show TangentSpace I p from v))

/-- **Math.** `chartedExpAt g p` is `ContMDiffAt 𝓘(ℝ, E) 𝓘(ℝ, E) 1` at `0`. -/
...
/-- **Math.** `chartedExpAt g p` is `ContDiffAt ℝ 1` at `0`. -/
...
/-- **Math.** The Fréchet derivative of `chartedExpAt g p` at `0` is the identity. -/
...
/-- **Math.** A convenient `HasFDerivAt` statement using the refl equiv. -/
...
/-- **Math.** The Banach inverse function theorem applied to `chartedExpAt g p` at
`0`, with derivative the identity continuous linear equiv. -/
private def chartedExpAtIFTHomeomorph
    (g : RiemannianMetric I M) (p : M) : OpenPartialHomeomorph E E :=
...
/-- **Math.** The IFT-produced inverse equals the `.symm` of the open partial
homeomorph. -/
...
/-- **Math.** The IFT-produced inverse is `ContDiffAt 1` at `chartedExpAt g p 0`. -/
...
/-- **Math.** A choice of open neighborhood `U` of `0 : E`. -/
private def niceSource (g : RiemannianMetric I M) (p : M) : Set E :=
  (Classical.choose (exists_nice_open_nhds (I := I) g p))

/-- **Math.** A choice of open neighborhood `W` of `chartedExpAt g p 0` in `E`. -/
private def niceSymmDomain (g : RiemannianMetric I M) (p : M) : Set E :=
  Classical.choose
    (Classical.choose_spec (exists_nice_open_nhds (I := I) g p))

private lemma niceSource_spec (g : RiemannianMetric I M) (p : M) :
...
private lemma niceSource_isOpen (g : RiemannianMetric I M) (p : M) :
    IsOpen (niceSource (I := I) g p) := (niceSource_spec (I := I) g p).1

private lemma zero_mem_niceSource (g : RiemannianMetric I M) (p : M) :
...
private lemma niceSource_smoothOn (g : RiemannianMetric I M) (p : M) :
    ContMDiffOn 𝓘(ℝ, E) I 1
      (fun v : E => (expMap (I := I) g p (show TangentSpace I p from v) : M))
      (niceSource (I := I) g p) :=
  (niceSource_spec (I := I) g p).2.2.2.1

private lemma niceSource_extSrc (g : RiemannianMetric I M) (p : M) :
    ∀ v ∈ niceSource (I := I) g p,
      (expMap (I := I) g p (show TangentSpace I p from v) : M)
        ∈ (extChartAt I p).source :=
...
/-- **Math.** The target in `M`: image of `niceSource g p` under `expMap g p`. -/
private def niceTarget (g : RiemannianMetric I M) (p : M) : Set M :=
  (fun v : E => (expMap (I := I) g p (show TangentSpace I p from v) : M))
    '' (niceSource (I := I) g p)

/-- **Math.** The image `chartedExpAt g p '' niceSource g p` is open in `E`. -/
...
/-- **Math.** The target characterisation. -/
...
/-- **Math.** For `q ∈ niceTarget g p`, `(extChartAt I p) q ∈ niceSymmDomain g p`. -/
...
/-- **Math.** The candidate local inverse: chart, then IFT inverse. -/
private def niceInvFun (g : RiemannianMetric I M) (p : M) : M → E :=
...
private lemma niceInvFun_mapsTo_niceSource
    (g : RiemannianMetric I M) (p : M) {q : M}
...
/-- **Math.** The inverse `niceInvFun g p` is `ContMDiffOn I 𝓘(ℝ, E) 1` on `niceTarget`. -/
...
/-- **Math.** The exponential map at `p`, packaged as a partial diffeomorphism
...
/-- **Math.** **The exponential map at `p` is a `C^1` local diffeomorphism at the
zero tangent vector.** -/
...
/-- **Math.** There exists an open neighborhood of `0 ∈ T_p M` on which `expMap g p`
