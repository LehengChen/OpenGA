...
/-- **Math.** **Chart-of-`TM` identification at a base point `q`, fibre component.**
...
/-- **Math.** **Chart-of-`TM` identification at a base point `q`, base component.**
For `p : TangentBundle I M` whose projection lies in the chart-source of
...
/-- **Math.** **Chart-of-`TM` identification at `⟨α, 0⟩` — full form.** The
...
/-- **Math.** **Chart-of-`TM` identification at `⟨α, 0⟩` — `chartFiberCoord` form.**
...
/-- **Math.** The chart of `TM` at a point `q` only depends on `q.proj`: it equals
...
/-- **Math.** **Chart-pushed lift decomposition.** For a curve `f : ℝ → TM` and a
...
/-- **Math.** **First component of the chart-pushed lift.** For times where
...
/-- **Math.** **Second component of the chart-pushed lift.** For times where
...
/-- **Math.** **Chart-pushed lift at `t = t₀`.** Self-application identity. -/
...
/-- **Math.** **Chart-pushed lift at a zero-section base.** When `f t₀ = ⟨α, 0⟩`,
...
/-- **Math.** **Headline bridge (chart-phase-ODE-conditional form).** Given a
...
/-- **Math.** **Closure of the bridge headline — chartPushedFlow ↔ witness curve
...
/-- **Math.** **Bridge headline against `maximalGeodesicChosenCurve`.** Given a
