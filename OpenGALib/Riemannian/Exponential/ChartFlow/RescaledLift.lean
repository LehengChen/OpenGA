import OpenGALib.Riemannian.Exponential.ChartFlow.ChartFlowToTangentLift
import OpenGALib.Riemannian.Exponential.ChartFlow.InverseManifoldChain
import OpenGALib.Riemannian.Exponential.ChartFlow.PreconnectedPropagation
import OpenGALib.Riemannian.Exponential.ChartFlow.ChartFlowData
import OpenGALib.Riemannian.Geodesic.MaximalInterval
import OpenGALib.Riemannian.Geodesic.MaximalRescaling
...
/-- **Math.** The **rescaled manifold lift**: invert the chart of `TM` at the zero
...
/-- **Math.** **Rescaling preserves the chart-target-interior product.** If
...
/-- **Math.** **Initial value of the rescaled lift.** Using the chart-flow's
...
/-- **Math.** **Projection identity.** When `Φ((x₀, v), t' * s)` lies in the
...
/-- **Math.** **Chart-source membership of the projection.** -/
...
/-- **Math.** The time-rescaling `s ↦ t' * s` maps `Ioo (-T/t') (T/t')` into
`Ioo (-T) T` when `t' > 0`. -/
...
/-- **Math.** **Chart-phase ODE for the rescaled chart-coordinate orbit.** If the
...
lemma rescaled_orbit_hasDerivAt_chartPhaseVF
    {g : RiemannianMetric I M} {p : M} {Φ : (E × E) × ℝ → E × E}
...
/-- **Math.** **Identification of a local lift with the rescaled lift near `s₀`.**
...
private lemma local_lift_eventuallyEq_chartFlowOrbitLiftRescaled
    (g : RiemannianMetric I M) (p : M) (v : E) (t' : ℝ)
    {Φ : (E × E) × ℝ → E × E} {s₀ : ℝ}
...
/-- **Math.** **`F_v^resc` is a local integral curve at every `s₀ ∈ Ioo (-T/t') (T/t')`.** -/
...
/-- **Math.** **Headline: `F_v^resc` is an `IsMIntegralCurveOn` on `Ioo (-T/t') (T/t')`.** -/
...
/-- **Math.** **The rescaled interval is preconnected.** Open ball in ℝ ⟹ convex ⟹
...
/-- **Math.** **`F_v^resc 0 = ⟨p, t' • v⟩` packaged together with the
...
/-- **Math.** **Identification of the rescaled lift's projection with
...
/-- **Math.** **Headline: projection at `s = 1` equals `expMap g p (t' • v)`.** When
