...
/-- **Math.** The cutoff field equals the genuine chart-phase field on the inner
...
/-- **Math.** Time-padded autonomous form of `chartPhaseVF`. -/
def chartPhaseVFAuto (g : RiemannianMetric I M) (α : M) :
    ℝ → (E × E) → E × E :=
  fun _ z => chartPhaseVF (I := I) g α z

@[simp] lemma chartPhaseVFAuto_apply
    (g : RiemannianMetric I M) (α : M) (t : ℝ) (z : E × E) :
    chartPhaseVFAuto (I := I) g α t z = chartPhaseVF (I := I) g α z := rfl

/-- **Math.** On the chart-interior product, `chartPhaseVF g α` is locally
...
/-- **Math.** **Chart-coordinate ODE uniqueness.** If two curves `c₁, c₂ : ℝ → E × E`
...
/-- **Math.** The chart-pushed flow's orbit through the base point `(x₀, v₀)`, as a
function of time. -/
...
/-- **Math.** The chart-pushed flow geodesic curve on `M`: pull back the first
...
/-- **Math.** **Initial value of the chart-pushed flow geodesic curve.** Provided
...
/-- **Math.** **Existence packaging of the chart-pushed flow geodesic curve.** For
...
/-- **Math.** The chart-pushed flow orbit `t ↦ Φ((x₀, v_chart), t)` satisfies the
...
/-- **Math.** **Chart-coordinate orbit uniqueness against the chart-pushed flow.**
...
/-- **Math.** **Bridge consequence in manifold terms.** Any manifold curve
...
/-- **Math.** The **bridge headline** in a packaged existence form: there exist a
