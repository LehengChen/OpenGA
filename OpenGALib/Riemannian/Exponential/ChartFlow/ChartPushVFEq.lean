import OpenGALib.Riemannian.Exponential.ChartFlow.ChartFlowGeodesicLink
import OpenGALib.Riemannian.Exponential.ChartFlow.ChartIdentification
...
import OpenGALib.Riemannian.Geodesic.SmoothFlow
import OpenGALib.Riemannian.Geodesic.Smoothness
...
/-- **Math.** The achart on `TM` (indexed by `ModelProd H E`) depends only on the
...
/-- **Math.** The chart at `⟨α, 0⟩` on `TM` source contains `q : TM` iff
...
/-- **Math.** For `q : TM` with `q.proj ∈ (chartAt H α).source`,
...
/-- **Math.** For `q : TM` with `q.proj ∈ (chartAt H α).source`, the value of
...
/-- **Math.** The chart change `tangentCoordChange I.tangent q ⟨α, 0⟩ q`, applied to
...
/-- **Math.** Specialisation: applied to `geodesicVectorFieldChart g α q`, the
chart change returns `geodesicVectorFieldChartFiber g α q`. -/
lemma tangentCoordChange_tangent_geodesicVF
    (g : RiemannianMetric I M) (α : M) (q : TangentBundle I M)
    (hq : q.proj ∈ (chartAt H α).source) :
    tangentCoordChange I.tangent q (⟨α, (0 : E)⟩ : TangentBundle I M) q
...
/-- **Math.** When `(f 0).proj = α`, the achart of `TM` at `f 0` equals the achart
at `⟨α, 0⟩`. -/
...
/-- **Math.** When `(f 0).proj = α`, `tangentCoordChange I.tangent q (f 0) q`
...
/-- **Math.** **`chartPushVF` in terms of `geodesicVectorFieldChartFiber`.** When
...
/-- **Math.** The chart-α fibre form of the geodesic vector field at `f t` matches
...
/-- **Math.** **Headline identification.** When `(f 0).proj = α` and
...
/-- **Math.** When `(f t₀).proj = α`, the achart of `TM` at `f t₀` equals the
achart at `⟨α, 0⟩`. -/
...
/-- **Math.** When `(f t₀).proj = α`, `tangentCoordChange I.tangent q (f t₀) q`
coincides with `tangentCoordChange I.tangent q ⟨α, 0⟩ q`. -/
...
/-- **Math.** **`chartPushVF` in terms of `geodesicVectorFieldChartFiber`, general
...
/-- **Math.** The chart-α fibre form of the geodesic vector field at `f s` matches
the chart-phase vector field evaluated at the chart-pushed lift's
...
/-- **Math.** **Headline identification, general base time.** When
...
/-- **Math.** **Chart-phase derivative of the chart-pushed lift.** When the lift
...
/-- **Math.** **Chart-phase derivative + chart-target-interior condition.** Same
...
/-- **Math.** **Unconditional bridge against a witness curve.** Given a manifold
...
theorem chartPushedFlow_eq_witness_curve_eventually_unconditional
    (g : RiemannianMetric I M) (p : M) (v_chart : E)
    {γ : ℝ → M}
    {f : ℝ → TangentBundle I M}
...
/-- **Math.** **Unconditional bridge against `maximalGeodesicChosenCurve`.** Given
