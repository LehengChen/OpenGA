...
import OpenGALib.Riemannian.Exponential.ChartFlow.UniformExistence
...
/-- **Math.** The manifold-valued lift of the chart-pushed flow orbit: invert the
...
/-- **Math.** Membership of the chart-of-`TM`'s extended-chart target. For
...
/-- **Math.** The inverse of the chart of `TM` at `⟨p, 0⟩` evaluated at a chart-
...
/-- **Math.** The chart of `TM` at `⟨p, 0⟩` applied to its inverse at a chart-
...
/-- **Math.** **Initial value of the lift.** Provided the chart-flow's
...
/-- **Math.** **Projection identity.** When the orbit lies in the chart-target
...
/-- **Math.** **Chart-source membership of the projection.** The lift's projection
...
/-- **Math.** **Chart-pushed lift identity.** When the orbit lies in the chart-
...
/-- **Math.** **Continuity of the lift on the uniform interval.** When the orbit
...
/-- **Math.** For each `v`, there exists a manifold-valued curve `g_v : ℝ → TM`
...
/-- **Math.** **The lift is a local integral curve at `0`.** Under the chart-flow
...
/-- **Math.** **Headline R.D.1 — manifold lift on the uniform interval.** There
