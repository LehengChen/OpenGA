...
/-- **Math.** **Uniform inner-ball confinement.** From joint `C^1`-ness of `Φ` at
...
/-- **Math.** **Headline chart-coordinate uniform existence.** There exist a chart-
...
set_option linter.unusedVariables false in
/-- **Math.** **Uniform orbit ODE on `Ioo (-T) T`.** For each `v ∈ ball (0 : E) ρ`
...
lemma orbit_hasDerivAt_chartPhaseVF_uniform
    ...
    {Φ : (E × E) × ℝ → E × E}
    (hΦ_ILF : Analysis.ODE.Flow.IsLocalFlow
...
/-- **Math.** **Headline uniform chart-phase ODE.** Combining the two preceding
...
/-- **Math.** **Per-`v` orbit-projection identification, local form.** For each
...
/-- **Math.** **Headline R.C — uniform existence interval.** For a smooth
