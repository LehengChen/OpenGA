...
/-- **Math.** **Chart-`⟨α, 0⟩`-pushed derivative of a local integral curve.** For
...
theorem eventually_hasDerivAt_chartPhaseVF_at_zero_section
    {g : RiemannianMetric I M} {α : M} {s₀ : ℝ}
...
/-- **Math.** **Local existence of a tangent-bundle integral curve at a given
...
/-- **Math.** **Identification of the local lift with `F_v` near `s₀`.** Under the
...
/-- **Math.** **`F_v` is a local integral curve at every `s₀ ∈ Ioo (-T) T`.** Under
...
/-- **Math.** **Headline: `F_v` is an `IsMIntegralCurveOn` on `Ioo (-T) T`.** -/
...
/-- **Math.** **Headline R.D.3.a — `F_v` is an integral curve on the full uniform
