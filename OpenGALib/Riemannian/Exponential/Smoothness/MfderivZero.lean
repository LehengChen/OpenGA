import OpenGALib.Riemannian.Exponential.Smoothness.AtZero
...
open Analysis.ODE
open Analysis.ODE.Flow

section ChartPhaseVFLinearization

variable [I.Boundaryless]

/-- **Math.** A single scalar summand
...
private lemma chartChristoffelContraction_hasFDerivAt_zero
  ...
  have hsum : HasFDerivAt
    ...
    have hscalar_x0 : scalar (x, (0 : E)) = 0 := by
      ...
      rw [h2]; ring
    let Lk : ℝ →L[ℝ] E :=
      (ContinuousLinearMap.id ℝ ℝ).smulRight (Module.finBasis ℝ E k)
    have hLk : HasFDerivAt (fun s : ℝ => s • (Module.finBasis ℝ E k))
        Lk (scalar (x, (0 : E))) := by
    ...
    have hfn_eq2 :
        (fun s : ℝ => s • (Module.finBasis ℝ E k)) ∘ scalar =
          (fun z : E × E => scalar z • (Module.finBasis ℝ E k)) := by
      funext z; rfl
    rw [hfn_eq2] at hcomp
...
/-- **Math.** The chart-phase vector field `chartPhaseVF g α (x, w) = (w, -Γ(w,w)(x))`
...
/-- **Math.** On a neighborhood of `(x, 0)` lying in the inner ball of the cutoff
...
/-- **Math.** The cutoff chart-phase vector field has Fréchet derivative
...
/-- **Math.** **Combined chart-flow data.** For a smooth Riemannian metric `g` and
...
/-- **Math.** **The manifold derivative of `expMap g p` at the zero tangent vector is
