...
import OpenGALib.Riemannian.Exponential.Smoothness.ChartFlowVelocitySlice
...
/-- **Math.** The rescaled orbit of the V.4 flow at scale `t'` and initial fibre
...
/-- **Math.** Initial value: the rescaled orbit at `s = 0` is `(x₀, t' • v)`,
...
/-- **Math.** **Rescaled orbit satisfies the chart-phase ODE.** Given that the
...
/-- **Math.** The chart-phase vector field vanishes at the zero section: at
...
/-- **Math.** **Zero-section orbit constancy (eventually).** The V.4 flow's orbit
...
lemma chartFlow_zero_section_eventually_const
    ...
    (hΦ : Analysis.ODE.Flow.IsLocalFlow
...
/-- **Math.** For `t'` near `0` (in the flow's time interval), the value
`Φ((x₀, 0), t')` equals `(x₀, 0)`. -/
...
/-- **Math.** **Chart-coord rewrite of the candidate at general time.** For `t'`
...
/-- **Math.** **Chart-coord `C^1` smoothness of the candidate at general time.**
...
/-- **Math.** **Manifold-side identification predicate.** For `Φ : (E × E) × ℝ → E × E`
...
/-- **Math.** **Discharge of `UniformChartFlowBridge`, conditional on
...
/-- **Math.** **Headline conditional form (consumes the named manifold-side
