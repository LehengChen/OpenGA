...
import OpenGALib.Riemannian.Exponential.Smoothness.ZeroSectionConstancy
...
/-- **Math.** The chart-image of the base point lies in the chart-target interior
...
/-- **Math.** **Existence packaging: V.4 chart-flow with `t'`-evaluation
...
theorem exists_chartFlow_combined_witness
  ...
  obtain ⟨b, r, ε, ρ, T, Φ, hr, hε, hρ_pos, hT_pos, hb_sub, hΦ_loc, hcd, hinit⟩ :=
    Riemannian.Geodesic.exists_chartPhase_contDiffOn_isLocalFlow_combined
      (I := I) (M := M) (g := g) (α := p)
...
/-- **Math.** **The named manifold-side data predicate.** For a smooth Riemannian
...
def HasChartFlowGeodesicMatchData (g : RiemannianMetric I M) (p : M) : Prop :=
...
/-- **Math.** **Reduction.** If there exists a V.4-compatible chart-flow `Φ` and
...
/-- **Math.** Given a witness of `HasChartFlowGeodesicMatchData g p`, the exponential
