import OpenGALib.Riemannian.Exponential.ChartFlow.RescaledLift
...
/-- **Math.** **Unconditional discharge.** For a smooth Riemannian metric `g` on a
...
/-- **Math.** For a smooth Riemannian metric `g` on a boundaryless smooth manifold `M`
