...
/-- **Math.** The `v`-slice of a jointly-`C^1` chart-flow `Φ` at a fixed time `t*`:
...
/-- **Math.** The first-component projection of the chart-flow's `v`-slice is
`C^1` at `v = 0`. -/
...
/-- **Math.** The manifold-valued candidate map produced from a chart-flow `Φ`:
...
/-- **Math.** At `t' = 0` and `v = 0`, the candidate returns `p` when the chart-flow
...
/-- **Math.** **Eventually-near-`v = 0` chart-rewrite of the candidate at `t' = 0`.**
...
/-- **Math.** **Chart-coordinate `C^1` smoothness of the candidate at `t' = 0`.**
...
/-- **Math.** **Headline existence: chart-pushed flow with `v`-slice `C^1`
...
theorem exists_chartFlow_slice_contDiffAt_zero
  ...
  obtain ⟨_b, ρ, T, Φ, hρ_pos, hT_pos, _hb_sub, hcd, hinit⟩ :=
    Riemannian.Geodesic.exists_chartPhase_contDiffOn_isLocalFlow
...
/-- **Math.** **Headline existence: manifold-valued candidate with chart-coordinate
...
/-- **Math.** **Continuity of the manifold-valued candidate at `v = 0`.** -/
...
/-- **Math.** **Manifold-side `ContMDiffAt 1` smoothness of the candidate at
...
/-- **Math.** **Manifold-side existence of a `ContMDiffAt 1` candidate at `v = 0`.**
