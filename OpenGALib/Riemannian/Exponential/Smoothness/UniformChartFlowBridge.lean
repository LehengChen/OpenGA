...
/-- **Math.** The chart-phase rescaling on `E × E`: scale the velocity component
by `a`. -/
...
/-- **Math.** **Chart-phase vector field at a rescaled point.** The chart-phase
...
/-- **Math.** **Rescaled-orbit chain-rule chart-phase derivative.** If a curve
...
/-- **Math.** **Packaged uniform-in-`v` chart-flow bridge at a small time `t'`.**
...
for `w` near `0`.
-/
def UniformChartFlowBridge (g : RiemannianMetric I M) (p : M) : Prop :=
...
/-- **Math.** **`expMap_contMDiffAt_zero_of_uniformChartFlowBridge` via the uniform chart-flow bridge.**
...
/-- **Math.** **Existence-form variant.** The headline packaged as taking an
explicit `∃`-statement. -/
...
/-- **Math.** At `v = 0` and `t' = 0`, the chart-flow candidate value matches
