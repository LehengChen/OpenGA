...
/-- **Math.** The first component of the chart-pushed lift coincides with the
...
/-- **Math.** Specialisation: when the projection of `f` equals a given curve `γ`,
...
/-- **Math.** At the base time `t = t₀`, the chart-pushed phase-space vector field
...
/-- **Math.** For any tangent vector `v : TangentSpace I p` and base time `t₀ : ℝ`,
...
/-- **Math.** **First-derivative clause.** If `f` is a chart-`γ(t₀)`-centred
...
/-- **Math.** **Eventually first-derivative clause.** On a neighbourhood of `t₀`,
...
/-- **Math.** **Conditional chart-centred bridge.** Given the autonomous
...
/-- **Math.** **Eventually-form of the autonomous phase identity** for a
...
/-- **Math.** **Unconditional chart-centred bridge.** If `f` is a chart-`γ(t₀)`-
...
/-- **Math.** **Existence form of the chart-centred bridge.** For any manifold
...
/-- **Math.** **`IsGeodesicAt` bridge, chart-centred case.** A local geodesic
...
/-- **Math.** **Local invariance of `HasGeodesicEquationAt`.** If two curves
...
/-- **Math.** **Construction of a chart-`γ(t₀)`-centred lift agreeing with the
...
/-- **Math.** **`IsGeodesicAt → HasGeodesicEquationAt`, conditional on cross-VF
