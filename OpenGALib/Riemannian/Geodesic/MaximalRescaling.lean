...
import OpenGALib.Riemannian.Exponential.ChartFlow.ChartPushVFEq
import OpenGALib.Riemannian.Exponential.Defs
import OpenGALib.Riemannian.Exponential.Smoothness.UniformChartFlowBridge
import OpenGALib.Riemannian.Exponential.ChartFlow.UniformUniqueness
import OpenGALib.Riemannian.Geodesic.GeodesicEquationFromIntegralCurve
...
/-- **Math.** **Fibre coordinate at a self-application point.** The chart-α fibre
...
/-- **Math.** **Initial value of the chart-pushed lift.** When `f 0 = ⟨p, v⟩`,
...
/-- **Math.** **Rescaled chart-pushed lift.** Given `f_v : ℝ → TM` with the
...
/-- **Math.** **Initial value of the rescaled chart-coord orbit.** Plugging in
...
/-- **Math.** **Chart-pushed lift of `f_av` satisfies the chart-phase ODE with
...
lemma chartPushLift_av_phaseVF_and_target_interior
    {g : RiemannianMetric I M} {p : M} {a : ℝ} {v : E}
...
/-- **Math.** **Rescaled chart-pushed orbit with target-interior condition near
...
/-- **Math.** **Chart-coord ODE uniqueness, rescaled vs `c_av` form.** If `f_v`
...
/-- **Math.** **First-component decomposition of the rescaled chart-pushed orbit.**
...
/-- **Math.** **Eventually-manifold rescaling for the projected curves.** From the
...
/-- **Math.** **Eventually-rescaling for the chosen curve.** If
...
theorem maximalGeodesicChosenCurve_rescale_eventually
    (g : RiemannianMetric I M) (p : M) (v : TangentSpace I p) (a : ℝ)
...
/-- **Math.** **Manifold rescaling at time `1`, near-`0` form.** If the eventual
...
/-- **Math.** **Manifold rescaling at time `1`, via `maximalGeodesic`.** Same as
...
/-- **Math.** **Manifold rescaling at time `1`, eventually-`a`-small form.** There
