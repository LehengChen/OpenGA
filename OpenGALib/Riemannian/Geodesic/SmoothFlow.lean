...
import OpenGALib.Analysis.ODE.Flow.HigherRegularity.VariationalMapContDiffOnK
...
@[simp] lemma chartPhaseVF_apply (g : RiemannianMetric I M) (α : M)
    (z : E × E) :
    chartPhaseVF (I := I) g α z =
      (z.2, - chartChristoffelContraction (I := I) g α z.2 z.2 z.1) := rfl

@[simp] lemma chartPhaseVF_mk (g : RiemannianMetric I M) (α : M)
    (x v : E) :
...
lemma chartChristoffelContraction_scalarCoeff_contDiffOn
  ...
  have hCLM_i : ContDiff ℝ ∞ (((Module.finBasis ℝ E).coord i).toContinuousLinearMap) :=
    ((Module.finBasis ℝ E).coord i).toContinuousLinearMap.contDiff
  have hCLM_j : ContDiff ℝ ∞ (((Module.finBasis ℝ E).coord j).toContinuousLinearMap) :=
    ((Module.finBasis ℝ E).coord j).toContinuousLinearMap.contDiff
...
lemma chartChristoffelContraction_contDiffOn
  ...
  have hconst : ContDiffOn ℝ ∞
      (fun _ : E × E => (Module.finBasis ℝ E) k)
      ((interior (extChartAt I α).target) ×ˢ (Set.univ : Set E)) :=
    contDiffOn_const
...
theorem exists_chartPhase_isLocalFlow
  ...
  obtain ⟨r, ε, hr, hε, Φ, hΦ⟩ :=
    Analysis.ODE.Flow.exists_isLocalFlow_of_contDiffOn_univ
...
theorem exists_chartPhase_contDiffOn_isLocalFlow
  ...
  have hCDOn :=
    Analysis.ODE.Flow.contDiffOn_flow_of_isLocalFlow
...
theorem exists_chartPhase_contDiffOn_isLocalFlow_combined
  ...
  have hΦ' : Analysis.ODE.Flow.IsLocalFlow
        (chartPhaseVFTime (I := I) g α (x₀, v₀) b)
...
theorem exists_chartPhase_contDiffOn_isLocalFlow_combined_two
  ...
  obtain ⟨U, hU_open, hU_mem, hU_C2⟩ :=
    Analysis.ODE.Flow.exists_contDiffOn_flow_C2
...
theorem exists_chartPhase_contDiffOn_isLocalFlow_combined_nat
    [CompleteSpace E]
    (g : RiemannianMetric I M) (α : M) {x₀ v₀ : E} (n : ℕ) (hn : 1 ≤ n)
    (hx₀ : x₀ ∈ interior (extChartAt I α).target) :
    ∃ (b : ContDiffBump ((x₀, v₀) : E × E))
      (r : ℝ≥0) (ε : ℝ) (ρ T : ℝ)
      (Φ : (E × E) × ℝ → E × E),
  ...
  obtain ⟨U, hU_open, hU_mem, hU_Cn⟩ :=
    Analysis.ODE.Flow.exists_contDiffOn_flow_Cnat
