...
The named space `ℝPⁿ`, realized as the projectivization of `ℝ^(n+1)`, together
with its quotient topology, its standard affine charts, and its compactness.

The development is layered by how much structure each step needs, so downstream
code imports only the generality it uses:

1. **Carrier and topology** — `RealProjectiveSpace` is the projectivization
   `ℙ ℝ (EuclideanSpace ℝ (Fin (n + 1)))`, carrying the quotient topology
   inherited from the nonzero-vector quotient. This is the model-set layer every
   later construction sits on.
2. **Affine charts** — the scale-invariant coordinate ratios `[v] ↦ (vⱼ / vᵢ)`
   on the domain `Uᵢ` where the `i`th homogeneous coordinate is nonzero, packaged
   as the open partial homeomorphism `realProjectiveChart`. The chart payload is
   assembled from quotient-invariance lemmas for the domain predicate and the
   coordinate map, openness of `Uᵢ`, and continuity of both the chart and its
   homogeneous-insertion inverse.
3. **Atlas coverage and compactness** — every point lies in some affine chart
   (`realProjectiveSpaceHasStandardChart`), and `ℝPⁿ` is compact, realized as the
   continuous image of the unit sphere `𝕊ⁿ`.

## Main definitions

* `Manifold.RealProjectiveSpace` — `ℝPⁿ` as the projectivization of `ℝ^(n+1)`.
* `Manifold.realProjectiveChartDomain` — the standard affine domain `Uᵢ`.
* `Manifold.realProjectiveChart` — the standard affine chart as an open partial
  homeomorphism with `ℝⁿ`.

## Main results

* `Manifold.realProjectiveChartDomain_isOpen` — each affine domain `Uᵢ` is open.
* `Manifold.realProjectiveSpaceHasStandardChart` — every point lies in some
  standard affine chart domain.
* `Manifold.realProjectiveSpaceCompactSpace` — `ℝPⁿ` is compact.

Provenance: SmoothManifoldsLee a5f308c — Example_1_5, Exercise_1_7.
-/
...
/-- **Math.** The real projective space `ℝPⁿ`, realized as the projectivization of
`ℝ^(n+1)`. -/
abbrev RealProjectiveSpace (n : ℕ) :=
  ℙ ℝ (EuclideanSpace ℝ (Fin (n + 1)))

@[inherit_doc] notation "ℝP[" n "]" => RealProjectiveSpace n

/-- **Math.** The quotient topology on `ℝPⁿ` inherited from the projectivization
construction. -/
...
/-- **Math.** The standard affine domain `Uᵢ` in `ℝPⁿ`, the set of lines whose `i`th
...
/-- **Math.** On a homogeneous representative, membership in the affine domain `Uᵢ` is
the nonvanishing of the `i`th coordinate. -/
...
/-- **Math.** On a homogeneous representative, the affine chart map is given by the
standard coordinate ratios `vⱼ / vᵢ`. -/
...
/-- **Math.** The homogeneous vector obtained from affine coordinates by inserting `1`
in the `i`th slot. -/
def realProjectiveChartInvVector (n : ℕ) (i : Fin (n + 1)) (u : EuclideanSpace ℝ (Fin n)) :
    EuclideanSpace ℝ (Fin (n + 1)) :=
  WithLp.toLp 2 (Fin.insertNth i (1 : ℝ) u)

/-- **Math.** The homogeneous coordinates obtained by inserting `1` in the `i`th slot
...
/-- **Math.** The explicit inverse chart lands in the standard affine domain `Uᵢ`. -/
...
/-- **Math.** The standard affine domain `Uᵢ` is open in `ℝPⁿ`. -/
theorem realProjectiveChartDomain_isOpen (n : ℕ) (i : Fin (n + 1)) :
    IsOpen (realProjectiveChartDomain n i) := by
  -- `Uᵢ` is the quotient image of the open set where the `i`th coordinate is nonzero.
...
private theorem realProjectiveChart_left_inv (n : ℕ) (i : Fin (n + 1)) :
  ...
  -- Normalize a representative on projective space, dividing through by the `i`th coordinate.
  intro x
  apply Subtype.ext
...
private theorem realProjectiveChart_right_inv (n : ℕ) (i : Fin (n + 1)) :
    Function.RightInverse (realProjectiveChartInvToDomain n i)
      ((realProjectiveChartDomain n i).restrict (realProjectiveChartToFun n i)) := by
  -- The easy computation on the affine slice `[u, 1]`.
  intro u
  apply WithLp.ofLp_injective
  funext j
...
/-- **Math.** The standard affine chart on `ℝPⁿ`, sending a line `[v]` with nonzero
`i`th coordinate to its affine coordinates `(vⱼ / vᵢ)`, as an open partial
homeomorphism with `ℝⁿ`. -/
...
/-- **Math.** On a homogeneous representative, the standard affine chart is given by the
standard coordinate ratios `vⱼ / vᵢ`. -/
...
/-- **Math.** The inverse chart is the homogeneous-coordinate insertion `u ↦ [u, 1]`. -/
...
/-- **Math.** The inverse chart lands in the standard affine domain `Uᵢ`. -/
...
/-- **Math.** Every point of `ℝPⁿ` lies in the source of one of the standard affine
charts, equivalently in a chart whose coordinate map is a homeomorphism with `ℝⁿ`. -/
theorem realProjectiveSpaceHasStandardChart (n : ℕ) (x : ℝP[n]) :
...
/-- **Math.** Real projective space `ℝPⁿ` is compact, being the continuous image of the
unit sphere `𝕊ⁿ ⊆ ℝ^(n+1)`. -/
