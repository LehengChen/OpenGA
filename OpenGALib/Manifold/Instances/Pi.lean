...
/-!
# Finite product manifolds

The canonical manifold structure on a finite product `∀ i, M i`, modeled on the
finite product `ModelWithCorners.pi I` of the factor models, filling a gap left
open in Mathlib.

The development is layered by how much structure each step uses:

1. **Chart-level algebra** — `openPartialHomeomorph_pi_symm_trans_pi` and
   `mem_modelWithCorners_pi_source_iff` are purely about how the product chart
   and product model decompose coordinatewise; they need no smoothness and are
   the reusable bookkeeping every later step rewrites through.
2. **Differentiability** — `contDiffOn_pi_proj` and `contDiffGroupoid_pi` lift
   `C^r` regularity from the factor coordinate changes to the product, working
   over an arbitrary nontrivially normed field.
3. **Manifold instance** — `instIsManifoldPi` is the canonical owner instance
   over `ModelWithCorners.pi I`, assembled from the factorwise manifold
   structures via the layers above. `isManifold_pi` specializes it to the
   Euclidean topological-manifold case.

## Main definitions

* `Manifold.instIsManifoldPi` — the finite-product `IsManifold` instance over
  `ModelWithCorners.pi I`.

## Main results

* `Manifold.contDiffGroupoid_pi` — a finite product of `C^r` coordinate changes is `C^r`.
* `Manifold.isManifold_pi` — a finite product of topological manifolds is a topological manifold.

Provenance: SmoothManifoldsLee a5f308c — Example_1_8.
-/
...
omit [Fintype ι] in
/-- **Math.** The transition map between two product charts is the product of the
...
/-- **Math.** Membership in the product-source condition is equivalent to the
...
/-- **Math.** Each coordinate projection on a finite product model space is `C^r`. -/
...
/-- **Math.** A finite product of `C^r` coordinate changes is again `C^r`. -/
...
/-- **Math.** A finite product of manifolds is again a manifold, modeled on the
finite product `ModelWithCorners.pi I` of the factor models and built from the
factorwise charted-space and manifold structures. -/
...
/-- **Math.** A finite product of topological manifolds carries the canonical
topological manifold structure modeled on the finite product of the factor
Euclidean spaces. -/
theorem isManifold_pi
  ...
  -- This is the canonical finite-product manifold instance specialized to Euclidean models.
  infer_instance
